...
当前研究仍存在一定局限。首先，动态障碍和智能灌溉目前实现于 Python 高层规划环境中，Gazebo 内尚未生成真实移动行人或车辆，也尚未模拟真实药液雾化、风场漂移和沉积分布。其次，增强实验的 97\% 需求满足目标仅在部分随机种子下达到，喷洒开关虽然提高了任务真实性，但显著扩大了探索空间。再次，整图多区块任务虽然在课程引导后取得了稳定覆盖结果，但该结果仍依赖最短路径专家在训练早期提供示范，因此更适合被解释为“全局规划先验 + DRQN 局部策略”的分层框架，而不是纯端到端 DQN 独立解决整图喷洒。后续应采用更系统的课程学习、模仿学习、全局覆盖规划与局部 DRQN/Dueling DQN 避障控制相结合的分层策略，以提升动态增强任务中的多随机种子稳定性和真实 Gazebo 场景泛化能力。最后，当前 Rainbow DQN lite 尚未包含 C51 分布式价值学习和 NoisyNet 探索，后续可补充完整 Rainbow 结构，并进一步验证其在动态喷洒任务中的泛化能力。
